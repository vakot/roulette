...
export const PlayerSchema = new mongoose.Schema<PlayerModelType>({
  ...
  avatar: {
    ...
    default: ({ _id }: { _id: string }) =>
      `https://api.dicebear.com/7.x/miniavs/svg?seed=${_id}`,
  },
  message: { type: String, required: false },
}).plugin(timestamp)

const Player =
  mongoose.models.Player<mongoose.Model<PlayerModelType>> ||
  mongoose.model<PlayerModelType>('Player', PlayerSchema)
